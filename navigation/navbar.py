...
class Navbar(tk.Menu):
    def __init__(self, root):
        ...
        menu_file.add_command(label="Homepage", command=Callback(root.show_frame, HomePage))
        ...
        menu_orders.add_command(label="Create Buy/Sell Order", command=Callback(root.show_frame, OrderPage))
        ...
        menu_pricing.add_command(label="FX Prices/Charts", command=Callback(root.show_frame, PricesPage))

        # Operations
        menu_operations = tk.Menu(self, menu_styles)
        self.add_cascade(label="Operations", menu=menu_operations)
        menu_operations.add_command(label="Trade Bookings", command=Callback(root.show_frame, TradeBookingsPage))
        # Operations - Positions
        menu_positions = tk.Menu(menu_operations, menu_styles)
        menu_operations.add_cascade(label="Positions", menu=menu_positions)
        menu_positions.add_command(label="View Positions", command=Callback(root.show_frame, PositionsPage))
        menu_positions.add_command(label="Position Reconciliation", command=Callback(root.show_frame, PositionsRecPage))
