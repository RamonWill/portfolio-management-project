...
class NavBar(tk.Menu):
    def __init__(self, parent):
        ...
        menu_file.add_command(label="Homepage",
                              command=lambda:
                              parent.show_frame(views.HomePageFrame))
        ...
        menu_orders.add_command(label="Create Buy/Sell Order",
                                command=lambda:
                                parent.show_frame(views.CreateOrdersFrame))
        menu_orders.add_separator()
        menu_orders.add_command(label="FX Algo Trading",
                                command=lambda:
                                parent.show_frame(views.AlgoTradingFrame))
        ...
        menu_pricing.add_command(label="FX Prices/Charts",
                                 command=lambda:
                                 parent.show_frame(views.SecurityPricesFrame))
        ...
        menu_operations.add_command(label="Trade Bookings",
                                    command=lambda:
                                    parent.show_frame(views.TradeBookingsFrame))
        ...
        menu_positions.add_command(label="View Positions",
                                   command=lambda:
                                   parent.show_frame(views.CurrentPositionsFrame))
        menu_positions.add_command(label="Position Reconciliation",
                                   command=lambda:
                                   parent.show_frame(views.PositionRecFrame))
